...
(function () {
   ...
   */
  var FirebaseAuthMigrator = function FirebaseAuthMigrator(app) {
    ...
    return new firebase.Promise(function (resolve, reject) {
      ...
      req.onreadystatechange = function () {
        ...
        if (req.status !== 200) {
          ...
          reject(new firebase.auth.Error("invalid-user-token", "Invalid auth token."));
          return;
        }
        var newToken = req.response.token;
        if (!newToken) {
          reject(new firebase.auth.Error("network-request-failed", "Invalid response from Firebase."));
          return;
        }
        resolve(newToken);
      };
      req.onerror = function () {
        reject(new firebase.auth.Error("network-request-failed", "Unable to verify auth token."));
      };
